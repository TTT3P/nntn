...
| **B9** | 🟠 P1 | hub-delivery draft hold bag_ids ที่ session อื่นส่งไปแล้ว → form mismatch · submit fail (พบ 30/04 draft `c37e481a`) | manual UPDATE meat_lines ตัด stale ออก (Platform ทำให้ไทน์เรียบร้อยแล้ว 30/04 09:30) | hub-delivery.html `loadDraft` → cross-check `cw.status='✅ In Stock'` ก่อน fill form · auto-prune stale + popup "ตัด N ถุงที่ delivered ไปแล้วโดย session อื่นออก · เหลือ X ถุงพร้อมส่ง" |
